...
internal sealed class LayoutEditHistoryService
{
    ...
    internal bool CanUndo(LayoutProfileKey key) =>
        _history.TryGetValue(key, out var snapshots) && snapshots.Count > 0;

    internal bool TryUndo(LayoutProfileKey key, out LayoutProfile profile)
    {
    ...
    internal void Clear(LayoutProfileKey key)
    {
        _history.Remove(key);
    }
}

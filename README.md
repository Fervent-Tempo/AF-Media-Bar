...
| 容器布局 | 长条内可组合多个常驻/悬停切换容器；长条外可放置边缘折叠容器，折叠时只保留触发区域；设置编辑器采用插入区、画布、属性区三栏布局 |
| 信息密度 | 悬停状态可使用歌曲名和歌手两行组件，最多显示行数只控制文本内部换行，不改变容器尺寸 |
| 自动隐藏 | 可在所有媒体会话均停止播放时隐藏；边缘折叠容器在鼠标靠近时展开并显示其内容 |
...
- [ ] 媒体进度条展示。
- [ ] UI美化、提供多种预制主题。
- [x] UI布局编辑器，支持拖放拼贴、长条内多容器、四边折叠容器、主要属性、撤销和单档案重置。
- [ ] 配置导出与分享。
- [ ] 添加新手教程引导。

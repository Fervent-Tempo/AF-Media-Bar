...
| 容器布局 | 设置页正在使用整数逻辑网格和容器/组件网格定位，支持单击创建 1×1、拖动创建矩形和四边缩放；schema 4 编辑器仍在完善，部分边界、折叠和 DPI 场景可能需要修复 |
| 信息密度 | 悬停状态可使用歌曲名和歌手两行组件，最多显示行数只控制文本内部换行，不改变容器尺寸 |
| 自动隐藏 | 可在所有媒体会话均停止播放时隐藏；折叠容器使用锚点容器和公共边展开，但 schema 4 四向折叠仍属于待验收能力 |
...
- schema 4 网格编辑器仍在持续修复；真实 Windows 下的四向折叠、DPI 指针对齐、窗口外接近和迁移恢复尚未作为稳定能力承诺。

...
| 托盘音频控制 | 单击自有托盘图标打开输出设备、空间音效状态和应用音量面板 |
| 音频设备 | 在面板中查看并切换默认输出设备；滚轮预览并在停止滚动后应用 |
| 应用音量 | 面板列出当前端点的应用音频会话；托盘滚轮按 2% 调节当前媒体应用音量 |
...
| 在面板输出设备行滚轮 | 预览设备，停止滚动约 450ms 后切换 |
| 在托盘图标上滚轮 | 按设置调节当前媒体音量或切换输出设备；按住 Shift 临时反转行为 |
| 点击空间音效行 | 打开 Windows 声音设置；第三方应用不能可靠代替系统切换 Dolby/DTS 模式 |

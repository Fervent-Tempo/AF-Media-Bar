...
| 窗口模式 | 在任务栏子窗口与悬浮窗口之间切换；悬浮窗口支持自由拖动、始终置顶和全屏隐藏 |
...
> [!NOTE]
> 在“依附任务栏模式”下，播放器会跟随 Windows 自动隐藏任务栏的显示与收回动画；“悬浮窗口模式”不依赖任务栏位置或自动隐藏状态。
...
| 切换到悬浮窗口模式 | 将播放器放到桌面工作区的任意位置；切换模式时会重建窗口以保证显示正确 |

...
| 来源交互 | 点击封面、标题与歌词可分别绑定播放暂停、切回媒体应用或打开完整层；右键切换来源；“媒体与通知”里可按应用启用 SMTC 允许列表 |
| 实时歌词 | 由 Web 歌词引擎在任务栏直接呈现，支持译文、音译与双行对齐；仅在来源提供真实逐字时间轴时逐字擦亮，行级歌词只按行切换；按网易云音乐、LRCLIB、QQ 音乐、酷狗音乐、汽水音乐依次匹配来源 |
| 任务栏适配 | 挂载为任务栏子窗口并自动避让图标与系统区域；封面保持自身宽高比，放不下的文字轮转滚动；可指定目标屏幕，无播放时可自动隐藏 |
| 外观与主题 | 字体、字号、浅色/深色、窗口材质与材质浓度全局统一；主题色可跟随系统强调色或自选 |
| 界面语言 | 简体中文、繁体中文与英文；默认跟随 Windows 显示语言，切换后立即生效 |
| 悬停层与完整层 | 悬停时显示控制层并模糊/淡化文字区；完整层可开关分区与组件、双预设切换 |
| 音频 | 切换默认输出设备、按 2% 调节当前媒体应用音量、查看空间音效；托盘图标的左键与滚轮可改绑 |
| 快速启动 | 没有可接收的媒体时点音符打开启动列表：滚轮循环预览，停止约 1.2 秒后启动最后一项 |
| 频谱与性能 | 四种频谱样式与可调的柱数、高度、刷新率；任务栏尾部轮换显示内存、CPU、GPU 与本程序内存，点击打开任务管理器 |
| 曲目切换通知 | 曲目切换并开始播放时显示曲目，位置、时长与显示器可调；按来源与标题识别切换，不读取播放队列 |
| 低性能降级 | 检测到软件渲染或低性能时自动关闭装饰性动效、滚动文字、频谱缓动与模糊效果 |
| 诊断与维护 | 空闲、关屏与睡眠时按档回收内存，启动安定后再回收一次；设置页可手动压缩内存，并提供程序日志 |
...
- 需要 Microsoft Edge WebView2 Runtime；Windows 11 与仍受支持的 Windows 10 通常已预装，精简系统若缺失需先安装 Evergreen Runtime。
...
用户偏好与窗口状态保存在 `%LOCALAPPDATA%\AFMediaBar\settings.json`（带 schema 编号的 JSON、原子写入与备份恢复），同一版本内替换程序文件不会丢失设置；**跨版本升级时旧设置文件不会被读取**（1.1.1 → 1.2.0 会把旧文件改名留档为 `settings.json.unsupported-<时间戳>` 并回到默认值），「应用」页可打开设置文件夹。

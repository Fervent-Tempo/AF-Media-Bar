...
| 类别 | 功能 |
| --- | --- |
| 媒体控制 | 显示封面、标题与作者；点击封面播放/暂停；滚轮切换上一首/下一首；切换多个媒体来源 |
| 来源交互 | 点击标题或歌词切回媒体应用；右键媒体栏切换来源 |
| 实时歌词 | 默认单行居中显示；可在设置中启用双行并选择下一句/当前句翻译作为第二行 |
| 任务栏适配 | 自动识别横向/竖向任务栏；支持手动拖动与锁定、任务栏自动隐藏与全屏隐藏 |
| 窗口模式 | 任务栏和灵动岛共享横向、竖向两套布局；灵动岛可拖动到桌面任意位置并在边缘自动收起 |
| 外观设置 | 可分别选择西文与中文字体、调整字重、切换播放器文字颜色和增强可读性，并为整个程序指定自动/浅色/深色主题 |
...
| 托盘音频控制 | 单击自有托盘图标打开音频面板；悬浮和滚轮反馈使用 Windows 原生提示显示当前操作值 |
...
| 在托盘图标上滚轮 | 按设置调节当前媒体音量或切换输出设备，并更新原生提示；按住 Shift 临时反转行为 |

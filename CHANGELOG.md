...
- 布局档案改为任务栏与悬浮共享的横向、竖向两套；宿主模式与排布方向分开选择；布局 JSON 升级到 schema 3，旧档案自动迁移并保留校验钳制、原子写入、备份和损坏文件恢复。
- 设置窗口改为可拖放拼贴编辑器：长条内可组合常驻/悬停切换容器，边缘折叠容器只编辑展开内容；采用插入区、画布、属性区三栏工作区，拖放命中真实容器槽位，预览与运行时视觉一致，支持按档案隔离的撤销和单档案重置，容器可恢复默认行为。
- 组件属性面板优先显示主要选项，可只重置当前组件默认值；调色板直接显示歌曲名、歌手及具体播放控制名称。
- 新增歌曲名与歌手两行信息组件；媒体文本支持最大行数、宽度限制和省略，最多显示行数移入高级显示且不再改变容器高度。
- 增加封面、媒体文本、控制、指标、频谱和分隔线组件的尺寸与专属数值设置，并补齐三语言资源和搜索索引。
- 容器增加内容对齐选项（居中、靠前、靠后、填满）；悬停容器的靠近触发距离可在高级行为中调节（默认 48 DIP）；边缘折叠时完全隐藏展开内容，只保留触发区域。
- 长条空白区域固定可拖动，任务栏模式开始拖动会临时退出自动定位/锁定；输出设备和媒体音量组件支持鼠标滚轮；设置窗口控件改为响应式宽度。

### 修复

- 修复多行歌曲名仍被跑马灯改回单行，以及重置“上一首/歌手”组件时误变成“播放/歌曲名”的问题。
- 修复折叠边缘容器的拖动碰撞和多个悬停容器共用鼠标状态导致的闪烁；悬停离开切换改为单向淡入靠近内容、离开内容立即落地，避免动画结束后旧组件闪回。

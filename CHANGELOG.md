...
- 增加四套独立的任务栏/悬浮、横向/竖向布局档案，支持常驻、悬停切换和自动折叠容器。
- 设置窗口增加组件树、组件增删排序、启用/禁用、实时预览和按组件类型变化的属性面板。
- 布局 JSON 使用版本化、校验钳制、原子写入、备份和损坏文件恢复；旧设置首次启动自动迁移。
- 增加封面、媒体文本、控制、指标、频谱和分隔线组件的尺寸与专属数值设置，并补齐三语言资源和搜索索引。

### 兼容性

- 悬停离开态与自动折叠态禁止交互组件，避免不可见或离开状态触发媒体与设备操作。
- 布局尺寸由组件树估算，保留旧窗口行为作为弹窗锚点回退；Windows 10 1809+ 最低版本不变。

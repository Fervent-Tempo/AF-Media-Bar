...
- 布局数据当前使用 schema 4，并保留旧 schema 的迁移、备份和损坏文件恢复路径。
- 细网格编辑器仍在稳定化；四向折叠、DPI 指针对齐、窗口外接近、任务栏恢复和迁移后的真实 Windows 行为尚未作为完全验收的稳定能力承诺。
- 悬停离开态禁止交互组件；边缘折叠时完全隐藏展开内容，只保留触发区域，避免不可见状态触发媒体与设备操作。
- 布局档案及组件属性保存在 `%LOCALAPPDATA%\AFMediaBar\profiles\layout.json`；旧组件注册表设置只用于首次迁移。

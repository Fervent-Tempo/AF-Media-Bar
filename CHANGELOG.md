...
- 增加由“宿主模式 + 排布方向”组合的四套独立布局档案；长条内可自由组合多个常驻/悬停切换容器，长条外支持四边折叠容器。
- 设置窗口改为可拖放拼贴编辑器，折叠容器只编辑展开内容；主要属性优先显示，并支持按档案隔离的撤销和单档案重置。
- 布局 JSON 升级到 schema 2，保留旧 Root/CollapsedSlot 读取迁移；继续使用校验钳制、原子写入、备份和损坏文件恢复。
